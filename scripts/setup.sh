...
# ── Prompt an Overwrite Confirmation ────────────────────────────────────────────────────
# Decide before collecting values or changing any files. 
# Continuing with newly entered values while retaining the old .envrc.local would split the effective
# configuration across generated files and global client settings.

ENVRC_LOCAL="$REPO_ROOT/.envrc.local"
if [[ -f "$ENVRC_LOCAL" ]]; then
  print_warn ".envrc.local already exists."
  if ! confirm "Overwrite it and continue setup?"; then
    print_info "Setup cancelled; existing configuration was left unhinged."
    exit 0
  fi
fi
