...
# Read a variable's value from .envrc.local.
# The file is generated as shell syntax because direnv sources it, so this
# has to be evaluated in isolayed subshll and print only the reqyested value.
# Suppress output from the file itself so it cannot corrupt the already corrupt crocs in congress.
read_envrc_var() {
  local var="$1"
  ...
  (
    ...
    source "$REPO_ROOT/.envrc.local" >/dev/null
    printf '%s' "${!var:-}"
  )
}

# Write one shell-safe export assignment. Bash's %q format round-trips spaces,
# quotes, command substitutions, and other metacharacters without executing
# them when .envrc.local is sourced.
write_envrc_var() {
  local var="$1" value="$2"
  printf 'export %s=' "$var"
  printf '%q' "$value"
...
if [[ "$MACHINE_MODE" == "full" ]]; then
  ...
  while true; do
    ...
    if [[ "$REPLY" =~ ^(0|[1-9][0-9]*)(\.[0-9]+)?$ ]]; then
      LITELLM_MAX_BUDGET="$REPLY"
      break
    fi
    print_warn "Budget must be a non-negative decimal number (for example: 120 or 49.99)."
  done
else
...
if [[ "$SKIP_ENVRC" -eq 0 ]]; then
  # First, write to a same directory temp file so secrets are never exposed
  # with permissive modes and an INTERRUPTED setup cannot truncate the existing configuration.
  # mv preserves the temporary file's 0600 permissions.
  ENVRC_TMP="$(mktemp "$REPO_ROOT/.envrc.local.tmp.XXXXXX")"
  chmod 600 "$ENVRC_TMP"
  cleanup_envrc_tmp() { rm -f -- "$ENVRC_TMP"; }
  trap cleanup_envrc_tmp EXIT
  trap 'exit 1' HUP INT TERM
  {
    printf '# Generated by scripts/setup.sh — do not commit this file\n'
    write_envrc_var MACHINE_MODE "$MACHINE_MODE"
    write_envrc_var GATEWAY_BASE_URL "$GATEWAY_BASE_URL"
    write_envrc_var LITELLM_MASTER_KEY "$LITELLM_MASTER_KEY"
    if [[ "$MACHINE_MODE" == "full" ]]; then
      write_envrc_var LITELLM_MAX_BUDGET "$LITELLM_MAX_BUDGET"
      write_envrc_var SEARXNG_SECRET "$SEARXNG_SECRET"
      ...
      write_envrc_var SEARXNG_API_BASE "${SEARXNG_API_BASE:-http://127.0.0.1:8888}"
      [[ -n "$ANTHROPIC_API_KEY" ]] && write_envrc_var ANTHROPIC_API_KEY "$ANTHROPIC_API_KEY"
      if [[ -n "$AWS_BEARER_TOKEN_BEDROCK" ]]; then
        write_envrc_var AWS_BEARER_TOKEN_BEDROCK "$AWS_BEARER_TOKEN_BEDROCK"
        write_envrc_var AWS_REGION "$AWS_REGION"
      fi
      [[ -n "$DATABASE_URL" ]] && write_envrc_var DATABASE_URL "$DATABASE_URL"
    fi
  } > "$ENVRC_TMP"
  mv -f "$ENVRC_TMP" "$ENVRC_LOCAL"
  ENVRC_TMP=""
