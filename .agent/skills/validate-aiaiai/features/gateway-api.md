...
Coding agents call LiteLLM on port 4000. Claude Code uses the Anthropic Messages format. OpenCode and similar tools use the OpenAI Chat Completions format. Every request needs a key.
...
- Configure an agent with base URL `http://<LITELLM_HOST>:4000` and a key.
...
- **Mock key.** It mints `validate-mock` through `POST /key/generate` with metadata `allow_client_mock_response: true`. Check `mock-key-minted` passes when the response has a key.
