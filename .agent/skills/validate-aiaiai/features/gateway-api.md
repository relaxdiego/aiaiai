...
- **Models.** It sends `GET /v1/models` with the master key. Checks `lists-models-200` and `lists-<model>` for each of the four model names pass.
- **Pricing.** It sends `GET /model/info` with the master key. Check `model-info-has-copilot-pricing` passes when each of the four models has `input_cost_per_token`, `output_cost_per_token`, `cache_read_input_token_cost`, and `cache_creation_input_token_cost` above zero.
